...
const createWindow = () => {
  const mainWindow = new require$$3$1.BrowserWindow({
    ...
    height: 600,
    webPreferences: {
      preload: node_path.join(__dirname, "preload.ts")
    }
  });
  {
  ...
  require$$3$1.nativeTheme.on("updated", () => {
    if (store.get("theme") === "system") {
      mainWindow.webContents.send(
        "theme-changed",
        require$$3$1.nativeTheme.shouldUseDarkColors ? "dark" : "light"
      );
    }
  });
});
require$$3$1.ipcMain.handle("get-system-language", () => {
  return require$$3$1.app.getLocale();
});
